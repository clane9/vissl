...
@register_transform("GenericReplicate")
class GenericReplicate(ClassyTransform):
    """
    Adds the same data sample multiple times to the batch K times so that the batch.
    ...
    def __call__(self, x):
        ...
        for _ in range(self.num_times):
            output.append(deepcopy(x))
        return output

    @classmethod
    def from_config(cls, config: Dict[str, Any]) -> "GenericReplicate":
        """
        Instantiates GenericReplicate from configuration.

        Args:
            config (Dict): arguments for for the transform

        Returns:
            GenericReplicate instance.
        """
        num_times = config.get("num_times", 2)
        logging.info(f"GenericReplicate | Using num_times: {num_times}")
